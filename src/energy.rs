...
/// Compute per-replica average energy, and optionally per-spin forward interactions.
///
...
///   interactions: Option<Vec<f32>> of length (n_replicas * n_spins * n_dims)
...
pub fn compute_energies(
    ...
    let mut interactions = if with_interactions {
        Some(vec![0.0f32; n_replicas * n_spins * n_dims])
    } else {
        None
    };

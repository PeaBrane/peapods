...
class Ising():
    def __init__(self, lattice_shape, couplings='ferro', temp_list=np.geomspace(0.1, 10, 20)):
        ...
        couplings = [coupling for coupling in self.couplings.transpose([self.n_dims] + list(range(self.n_dims)))]
    ...
    def reset(self):
        ...
        self.energies = utils.get_energy(self.spins, self.couplings)
    ...
    # def __repr__(self):
    #     mag, energy = self.mag_aggregate / self.n_sweeps, self.energy_aggregate / self.n_sweeps

    #     rep = f'shape: {self.lattice_shape}, magnetization: {mag}, energy: {energy}'
    #     return rep
    ...
    def sweep(self, n_sweeps=1): 
        ...
        for sweep_id in range(n_sweeps):
            ...
            self.energies = utils.get_energy(self.spins[self.temp_ids], self.couplings)
    ...
    def parallel_tempering(self, n_sweeps, exchange_interval=1):
        for sweep_id in range(n_sweeps):
            self.sweep()

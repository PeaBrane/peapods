...
class Ising:
    def __init__(
        ...
        couplings_clone = [
            np.roll(self.couplings[..., i], 1, i) for i in range(self.n_dims)
        ]
    ...
    def reset(self):
        ...
        self.spins = -1 + 2 * rand(self.n_temps, *self.lattice_shape).round().astype(
            np.float32
        )
    ...
    def update(self, record=True, csd_update=False, compute_interactions=False):
        ...
        if record:
            ...
            if self.n_swept != 0 and self.n_swept % 2**3 == 0:
                self.binder_cumulant = 1 - (self.mags4_stat.average) / (
                    3 * self.mags2_stat.average**2
                )
                self.heat_capacity = (
                    self.energies2_stat.average - self.energies_stat.average**2
                ) / self.temperatures**2
    ...
    def sweep(self, mode="metropolis"):
        self.spins = sweeps.sweep(
            self.spins,
            self.couplings_doubled,
            self.neighbors,
            self.temperatures[self.temp_ids],
            mode=mode,
        )
...
class IsingReplicas(Ising):
    def __init__(self, lattice_shape, **kwargs):
        """
    ...
    def replica_cluster_update(self, update=True, record=True, cluster_mode="cmr"):
        ...
        interactions_pairs = np.stack(
            [interactions_pairs[0::2], interactions_pairs[1::2]], axis=1
        )

        for temp_id, (interaction, temp) in enumerate(
            zip(interactions_pairs, self.temperatures[::2])
        ):
...
class IsingEnsemble:
    def __init__(self, lattice_shape, n_ensemble=2, replicas=False, **kwargs):
        self.n_ensemble = n_ensemble

        if replicas:
            self.ising_ensemble = [
                IsingReplicas(lattice_shape, **kwargs) for _ in range(n_ensemble)
            ]
        else:
            self.ising_ensemble = [
    ...
    def get_csds(self):
        csds_ensemble = np.stack(
            [ising.get_csds(normalized=False) for ising in self.ising_ensemble]
        )
